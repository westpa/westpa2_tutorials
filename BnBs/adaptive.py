...
binsperdim=[13]   # You will have prod(binsperdim)+numberofdim*(2+2*splitIsolated)+activetarget bins total
pcoordlength=101 # length of the pcoord
maxcap=[22]	#for each dimension enter the maximum number at which binning can occur, if you do not wish to have a cap use inf
mincap=[4]  #for each dimension enter the minimum number at which binning can occur, if you do not wish to have a cap use -inf
...
splitIsolated=0     #choose 0 to disable the use of bottleneck walkers (not recomended)
#########
