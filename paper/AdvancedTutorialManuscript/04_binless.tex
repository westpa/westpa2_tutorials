...
To enable the use of the HDF5 framework in your own simulation, you may use this directory as an example. 
The location of the trajectory h5 files will need to be specified in the \verb|west.cfg| file, and the appropriate restart and topology files will need to be copied to the correct locations in the \verb|get_pcoord.sh| and \verb|runseg.sh| files. 
You will also need to make sure that the file extensions for any trajectory files are readable by \verb|mdtraj.load|, for example, Amber restart files must end in .ncrst to be used, which simply requires renaming. 
Even though the HDF5 framework may be in use, traditional trajectory files are still stored in the \verb|traj_segs/| directory. 
Feel free to delete those after every iteration in the \verb|post_iter.sh| file, which is located in the \verb|westpa_scripts/| directory.
...
To use the multitool, the \verb|west.h5| file for each WE simulation you want to combine should be placed within a numbered directory starting with \verb|01/|. 
If the simulations used a custom grouping function (such as in \verb|group.py|), you must also include that file in the top-level analysis directory. 
Note that in the provided tutorial files, you will have to decompress the provided HDF5 files, which (after decompression) will be automatically placed in numbered folders. 
For example, after decompression, the files will be organized as follows:
